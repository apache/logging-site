...
You can verify a distribution by following instructions:
...
Windows::
+
. Download https://downloads.apache.org/logging/KEYS[the release manager public keys]
. Use `gpg --import` to import the keys
. Verify signature files: `gpg --verify some.file.asc some.file`
. Verify checksums: `certUtil -hashfile some.file SHA512`
====
