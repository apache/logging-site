...
https://commons.apache.org/proper/commons-logging/[Apache Commons Logging (JCL)] is one of the oldest Java logging API
available.
...
According to Sonatype, Apache Commons Logging is used in https://central.sonatype.com/artifact/commons-logging/commons-logging[over 1 million artifacts], while the second place is taken by SLF4J with https://central.sonatype.com/artifact/org.slf4j/slf4j-api[almost 40 thousand artifacts].
...
Since version 1.2 only supported old generation logging backends out-of-the-box ({site-url}/log4j/1.x/[Log4j 1.x], https://avalon.apache.org[Avalon] and https://javalogging.sourceforge.net[Lumberjack]), the Java community developed many `LogFactory` implementations and complete Apache Commons Logging replacements:
...
* our own https://mvnrepository.com/artifact/org.apache.logging.log4j/log4j-jcl[`org.apache.logging.log4j:log4j-jcl`] (`LogFactory` implementation).
...
and to prevent regressions, add those dependencies to a https://maven.apache.org/enforcer/enforcer-rules/bannedDependencies.html[`bannedDependencies` Maven Enforcer rule].
...
* encouraged to migrate to Log4j 2.x Core (cf. https://logging.apache.org/log4j/2.x/manual/migration.html[migration guide]) or Logback,
